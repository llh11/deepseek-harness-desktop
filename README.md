...
- 打开官方设置后，导航在官方分区（通用 / 模型 / 插件 / Agent 预设）之后追加六个桌面分区：**内置服务、模型与多模态、账户与用量、Skill 加载器、MCP 插件、更新与关于**。
...
3. **对话流多模态集成**（1.3.1）：多模态能力直接集成在官方对话流中，无独立窗口或模块。模型切换器旁的「多模态 / 纯文本」标识实时反映当前模型能力；**纯文本模型在上传图片 / 视频的瞬间即被拦截**（点击上传、粘贴、拖拽三种入口全覆盖），并给出可一键切换的多模态模型建议；支持图片的模型则完全走官方原流程，上传、发送不受任何干扰。
...
- **账户余额**：对配置了 API Key 的 Provider 查询账户余额（目前支持 DeepSeek 官方账户，其它服务商显示"暂不支持"）。
- **用量统计**：本地翻译网关在转发请求时自动记录每次任务的 token 消耗（输入 / 输出分开计），按今日、近 7 日、累计三档汇总，并按模型分解；DeepSeek 系列模型给出人民币费用估算。支持一键清空记录。
- 只有勾选「经本地翻译网关接入」的 Provider 才纳入统计（网关是唯一能看见用量的位置）。
